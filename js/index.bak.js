...
const boxController = {
    ...
    update() {
        let all   = this.getAll()
        let temp  = parseInt(owo.storage.getItem('startTime'))
        let clock = owo.clock(temp)
        let today = owo.getFullDate(clock.year, clock.month, clock.date, clock.today)
        let last  = owo.getFullDate(clock.year, clock.month, clock.date + this.length - 1, clock.today + this.length - 1)

        if(temp === null) {
            owo.storage.setItem('startTime', clock.obj.getTime())
        }
        all.forEach((box, k) => {
    ...
    addPoints(count) {
        ...
        this.update()
        owo.storage.setItem('pointsCount', this.length)
    }
}
...
const createClockInterval = () => {
    ...
    return setInterval(() => {
        ...
        if(currentHour === hour) {
            ...
            // 更新时间进度条
            updateStyle.timeProgress(progress)
            return
        }
        currentHour = hour
        ...
        // 切换站台灯光
        updateStyle.switchStationLight((hour >= 19) && (hour <= 23) ? 1 : 0)
